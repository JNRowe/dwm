...
manage(Window w, XWindowAttributes *wa)
{
	...
	if (!strcmp(c->name, scratchpadname)) {
		...
		c->y = scratchpad_centre ? c->mon->wy + (c->mon->wh / 2 - HEIGHT(c) / 2) : 0;
	}
...
monocle(Monitor *m)
{
	...
	for (c = m->clients; c; c = c->next)
		if (ISVISIBLE(c)) {
			...
			if (hint.res_name && strcmp(scratchpadname, hint.res_name) != 0) {
				n++;
			}
		}
...
setup(void)
{
	...
	grabkeys();
	focus(NULL);
	if(stat(dwmfifo, &sb) == -1)
		mkfifo(dwmfifo, S_IRUSR | S_IWUSR);
